...
export default async function TermsPage() {
  ...
  const s7Items = t.raw('s7Items') as { label: string; text: string }[]
  const s8Items = t.raw('s8Items') as string[]
  const s10Items = t.raw('s10Items') as string[]
  const s11Items = t.raw('s11Items') as string[]
  const s13Items = t.raw('s13Items') as string[]
  const s14Items = t.raw('s14Items') as string[]
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8 text-gray-700 leading-relaxed">
          ...
          <section>
            ...
            <ul className="list-disc list-inside space-y-2 ml-2">
              {s10Items.map((item) => <li key={item}>{item}</li>)}
            </ul>
          </section>
          ...
          <section>
            ...
            <ul className="list-disc list-inside space-y-2 ml-2">
              {s11Items.map((item) => <li key={item}>{item}</li>)}
            </ul>
          </section>
          ...
          <section>
            ...
            <p className="mb-3">{t('s13Intro')}</p>
            <ul className="list-disc list-inside space-y-2 ml-2">
              {s13Items.map((item) => <li key={item}>{item}</li>)}
            </ul>
          </section>

          <section>
            <h2 className="text-2xl font-semibold mb-4">{t('s14Title')}</h2>
            <p className="mb-3">{t('s14Intro')}</p>
            <ul className="list-disc list-inside space-y-2 ml-2">
              {s14Items.map((item) => <li key={item}>{item}</li>)}
            </ul>
            <p className="mt-3">{t('s14Outro')}</p>
          </section>
          ...
          <section>
            ...
            <p className="mb-2">{t('s16Body')}</p>
            <p>
              <strong>{t('s16EmailLabel')}</strong> {CONTACT_EMAIL}<br />
              <strong>{t('s16PlatformLabel')}</strong> {PLATFORM_URL}
            </p>
          </section>
        </div>
      </div>
