...
| `supabase/functions/process-whatsapp/ai-agent.ts` | WhatsApp LLM Prompts and routing engine |

---

---

## 12. Mobile & Dashboard UX/UI (v6.7 - April 2026)

**Architecture Update:** Enhanced physical interaction and visibility control for the "Luis IA" FAB (Floating Action Button).

### 12.1 Visual Minimalism (Voice-Only UX)
- **Problem:** Transcript overlays were blocking mobile screen real-estate and occasionally "sticking" due to browser audio event mismatches.
- **Fix:** Removed the asynchronous transcript div from the DOM. Interaction is now purely guided by the **LED-State Button** (Red: Listening, Blue: Thinking/Speaking) and Luis's actual voice, aligning with high-end assistant standards (Siri/Alexa).

### 12.2 Full-Height Draggability (Mobile)
- **Problem:** The button was constrained to a small vertical range, often overlapping critical Dashboard stats or calendar entries.
- **Fix:** Implemented dynamic `window.innerHeight` calculation in `dragConstraints`. The FAB can now be moved from the very bottom to the very top of any mobile device, allowing the business owner to park Luis wherever it least interferes with their workflow.

### 12.3 Cloud-Synced Visibility Toggle
- **Problem:** Some users preferred a "Clean Desktop" experience without the floating button.
- **Fix:** 
    - **Settings Page:** Added a new "Smart Assistant" card in `app/dashboard/settings/page.tsx` with a `showLuisFab` toggle.
    - **DB Sync:** The preference is persisted in the `businesses.settings` JSON field (`uiSettings.showLuisFab`).
    - **Real-Time Bus:** The FAB listens to a `cronix:toggle-fab` CustomEvent. Switching the toggle in Settings hides/shows the button across the entire app instantly without a page refresh.

### 12.4 Database "Turbo" Hardening (Performance)
- **RLS Optimization:** Replaced heavy `auth.uid()` calls with `(select auth.uid())` subqueries to bypass plan-time overhead in PostgreSQL.
- **Function Security:** Hardened all Luis-related RPCs (e.g., `fn_book_appointment_wa`) with `SET search_path = ''` to prevent search path hijacking.
- **Clean Indices:** Purged duplicate FK indices and added covering indices for `passkey_challenges` and `expenses` to speed up authorization checks.

---

*Documentation generated by Antigravity V3 — Cronix AI Team. Version: Luis IA v6.7*
