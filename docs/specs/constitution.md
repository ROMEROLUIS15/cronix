...
*   **Umbrales de tamaño (DUROS, verificables).** Un archivo de código **no debe exceder ~300 líneas**; una función **no debe exceder ~50 líneas** ni mezclar más de una decisión/responsabilidad. Estos límites son señal de SRP: cuando se rozan, el archivo/función ya está acumulando responsabilidades y **debe descomponerse en módulos cohesivos** (NLU, orquestación, estado, persistencia, presentación), no "comprimirse". Un módulo que crece se parte por concepto, no por líneas arbitrarias. La orquestación delega a esos módulos; no los absorbe. (Excepción admisible y rara: tablas de datos/constantes declarativas sin lógica — deben ir en su propio archivo.)
