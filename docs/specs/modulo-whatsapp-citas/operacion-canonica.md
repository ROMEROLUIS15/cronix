...
- **AC-NLU-20 (fecha pasada):** si el cliente da una fecha **ya pasada** ("ayer", un día de un mes ido) en agendar o reagendar, el agente lo dice explícitamente ("El X ya pasó, ¿para qué día de hoy en adelante…?") en vez de descartarla en silencio y re-preguntar.
- **AC-NLU-21 (multi-intención saludo+reserva):** un saludo que **también** pide agendar/gestionar ("hola, quiero agendar mañana") **no** se responde solo con el saludo: `layerFaq` cede y la capa de booking atiende la reserva.
- **AC-NLU-22 (precio de un servicio puntual):** "¿cuánto cuesta Electrónica?" → responde el precio+duración de **ese** servicio (`serviceNamedIn` compartido, acento-insensible); sin nombrar servicio → lista completa. Misma lógica en el fast-path FAQ y en `layerServices`.
- **AC-NLU-23 (no inventar datos del negocio — fallback LLM):** el prompt del sistema (`buildMinimalSystemPrompt`) **inyecta la dirección real** (si existe) y prohíbe explícitamente inventar cualquier dato no presente (estacionamiento, métodos de pago, promociones, otra sede, wifi…): el LLM debe decir que no tiene ese dato y derivar al negocio. Cierra la superficie de alucinación de hechos del negocio que no tienen ruta determinista (ubicación/horario/servicios/precio sí la tienen).
