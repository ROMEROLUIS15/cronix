...
| D4 | Notificaciones "con error" | O1 | NO es por plantillas faltantes (las aprobadas `appointment_reminder` y `daily_owner_summary` existen y funcionan). Los síntomas eran los bugs de código D1/D3/D5. Limitación residual preexistente: la notificación POR EVENTO al dueño usa texto libre (sin fallback a plantilla) → fuera de la ventana de 24h de Meta no se entrega; campana/push no afectados | 🟢 reclasificado — sin plantilla faltante; mejora opcional: cablear la notif por evento al dueño a plantilla para entregar fuera de 24h |
