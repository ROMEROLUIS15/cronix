...
| Fecha | Cambio |
|---|---|
| 2026-06-22 | **Sección de Cobros saneada — los datos ahora se toman bien en toda la plataforma.** Auditoría encontró que el form de Cobros (`/dashboard/finances/new`) escribía una transacción suelta vía `createTransaction` directo (sin `appointment_id`, sin distribución), así que **contaba como ingreso pero no saldaba la deuda del cliente** (la deuda solo ve transacciones con `appointment_id`) → riesgo de doble conteo. Además `registerClientPayment` **descartaba el sobrante** si el monto excedía la deuda (o no registraba nada si no había deuda), y las escrituras de finanzas **no invalidaban el caché** `dashboard/stats`. Fix (decisión del dueño = "ambas según el caso"): el form ahora tiene **2 modos** — *Pago de cliente* enruta por `registerClientPayment` (distribuye sobre citas impagas, atómico, idempotente, y el sobrante se registra como ingreso suelto en vez de perderse) y *Otro ingreso* por la nueva action `registerOtherIncome` (transacción standalone, no salda cita). Ambos modos: `idempotency_key` (anti doble-cobro) + invalidación de `cache.invalidateKey(business,'dashboard','stats')` server-side. i18n ×6 (`finances.cobro.mode*`). **Invariante: el dinero nunca se pierde** (ingreso registrado == monto ingresado). 1613 tests + paridad verdes. NOTA: cobros al cliente sigue siendo zona sin spec de módulo (🔴). |
| 2026-06-22 | **Experimento de capacidad + harness de carga (local, free-tier-safe).** Nuevo `scripts/loadtest/` (seed set-based + `explain` de queries calientes + `load` de concurrencia sin k6, todo guardado a 127.0.0.1) y doc vivo [`docs/architecture/capacity-and-scaling.md`](../architecture/capacity-and-scaling.md). Resultado a 500 negocios/600k citas: queries del dashboard <17ms en estado estable, concurrencia degrada sin errores (codo ~25 concurrentes, 0 fallos hasta 100) → el cuello NO es el código/DB sino el compute/pool del tier. Hallazgo: fragilidad por estadísticas obsoletas (`get_clients_debts` ~6min sin `ANALYZE` vs ~16ms con; seeder ahora corre `ANALYZE`). Documentado el menú de escalado post-saturación con trade-offs (compute→pooler→caché→pre-agregación→réplicas). Pendiente: prueba del techo real del free tier contra proyecto desechable. |
...
| 2026-07-11 | **Migración de modelos Groq (deprecación Llama → GPT-OSS).** Groq anunció la deprecación de `llama-3.1-8b-instant` y `llama-3.3-70b-versatile` (decommission **2026-08-16**). Swap a los reemplazos oficiales: 8B → **`openai/gpt-oss-20b`**, 70B versatile → **`openai/gpt-oss-120b`** (ambos con tool-calling + JSON mode). Tocados todos los call-sites Groq: `process-whatsapp/groq-client.ts` (SMALL/LARGE), `voice-worker/providers/GroqProvider.ts`, `lib/ai/resilience.ts` (defaults), `lib/ai/providers/groq-provider.ts` (tiers fast/quality) y el reviewer `GroqReviewerLlm.ts` (**ambas copias**, parity) + judge de evals (`groq_rotator.py`). **Cerebras `llama3.3-70b` NO se toca** (proveedor distinto, no afectado). **Ajuste de correctitud del reviewer:** gpt-oss es modelo de razonamiento y sus tokens de reasoning cuentan contra el presupuesto → `max_tokens` 120→512 + `reasoning_effort:'low'` (si no, el JSON `{verdict,code,reason}` se truncaba). Tests unitarios afectados verdes (30), `tsc` limpio, docs vivos sincronizados (§7). **PENDIENTE antes de deploy:** re-validar evals del agente (`npm run test:evals:agent`) y latencia (gpt-oss razona más que llama-instant → vigilar budget Hobby 10s / TPM del free tier); considerar bajar a Qwen3 si la latencia/quota aprieta. |
