...
| 2026-06-13 | modulo-voice-agent §8: autoridad de veto del reviewer acotada al radio de daño — hard-block solo en destructivas (cancel/reschedule/delete); `book_appointment` degrada el veto a warn (ya cubierto por guards deterministas + findConflicts). Rúbrica v3 (regla 7): `create_client`→`book_appointment` es flujo normal, no duplicado/contradicción. Arregla REVIEWER_BLOCKED falso tras crear cliente y agendar. |
