...
/**
 * useVoiceAssistant — the FAB's voice state machine: recording (Web Speech API
 * on desktop, MediaRecorder + server STT on mobile/Firefox), the one-shot
 * voice-worker call, audio playback with mobile-autoplay unlocking, and the
 * silence-failure telemetry beacon.
 *
 * Extracted verbatim from VoiceAssistantFab — behaviour unchanged. The audio /
 * recording pipeline is intricate and validated against real mobile traffic, so
 * this is a pure move (same refs, same effects), not a rewrite.
 */
export function useVoiceAssistant(supabase: SupabaseClient, queryClient: QueryClient): VoiceAssistant {
  ...
  return { state, volume, handleClick }
}
