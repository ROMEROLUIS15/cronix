...
/**
 * The ONLY place that talks to ai_memories_v2.
 *
 * Reads go through match_ai_memories_v2 RPC (tenant filter applied
 * in SQL before the vector scan — correctness + perf).
 *
 * Writes are direct inserts; service_role bypasses RLS, so the
 * WHERE business_id contract is enforced explicitly in every query.
 */
